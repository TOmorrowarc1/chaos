...
## 1. Concept: Virtual Memory

### Paging

Physical RAM is divided into 4 KiB frames (pages). Each frame has a physical address (`PhysAddr`). The kernel manages which frames are free and which are allocated by the frame allocator.

A CPU with an MMU translates every address the program sees (virtual address, `VirtAddr`) to a physical address via a **page table**. The page table is a multi-level tree in memory, indexed by virtual address, whose leaves are page table entries (PTEs). Each PTE stores:
